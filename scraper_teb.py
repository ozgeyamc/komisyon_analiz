"""
TEB "Ürün ve Hizmet Ücretleri" sayfasını çeken scraper modülü.
"""
...
def scrape_teb(url: str = TEB_URL) -> List[UcretSatiri]:
    ...
    print(f"[teb] {url} adresinden veri çekiliyor...", file=sys.stderr)
    ...
    if not tum_satirlar:
        raise ScraperError("TEB sayfasında tablo var ama hiç veri satırı çekilemedi.")

    tum_satirlar = sorted(tum_satirlar, key=lambda s: (s.kategori, s.masraf))
    print(f"[teb] Toplam {len(tum_satirlar)} satır bulundu.", file=sys.stderr)
    return tum_satirlar
