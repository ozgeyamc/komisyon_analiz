"""
QNB Finansbank "Ürün ve Hizmet Ücretleri" sayfasını çeken scraper modülü.
"""
...
def scrape_qnb(url: str = QNB_URL) -> List[UcretSatiri]:
    from playwright.sync_api import sync_playwright
    from bs4 import BeautifulSoup

    print(f"[qnb] {url} adresinden veri çekiliyor...", file=sys.stderr)
    ...
    print(f"[qnb] Toplam {len(tables)} <table> bulundu", file=sys.stderr)

    if not tables:
        print("[qnb] TABLO YOK — Sayfa yapısı analiz ediliyor:", file=sys.stderr)
        body = soup.find("body")
        if body:
            text = body.get_text(separator="\n", strip=True)
            print(f"[qnb] Sayfa metni (ilk 1000 kar):\n{text[:1000]}", file=sys.stderr)
        divs = soup.find_all("div", class_=True)
        classes = set()
        for d in divs[:200]:
            for c in d.get("class", []):
                classes.add(c)
        print(f"[qnb] Bulunan div class'ları: {sorted(classes)[:50]}", file=sys.stderr)
        raise ScraperError("QNB sayfasında hiç <table> bulunamadı.")
    ...
    if not tum_satirlar:
        raise ScraperError("QNB sayfasında tablo var ama hiç veri satırı çekilemedi.")

    tum_satirlar = sorted(tum_satirlar, key=lambda s: (s.kategori, s.masraf))
    print(f"[qnb] Toplam {len(tum_satirlar)} satır bulundu.", file=sys.stderr)
    return tum_satirlar
